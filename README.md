...
2. **Despliegue de la migración a "solo Meta" — pendiente de probar en producción.** El código está escrito y pasa `node --check`, pero no se ha probado un mensaje real de punta a punta con este camino nuevo (memoria en D1, pausa automática, texto suelto). Antes de confiar en él: completar el setup de arriba, mandar un mensaje de prueba de texto, uno de foto directa y uno de respuesta a historia, y confirmar que cada uno se responde UNA sola vez y que el historial persiste entre mensajes (probar con "y en negro?" después de pedir un modelo).

3. **Pausa automática — sin probar.** Depende de que el webhook esté suscrito a `message_echoes` (ver setup arriba) y de que Meta realmente entregue el eco con `recipient.id` = cliente. Probar: un asesor responde a mano un mensaje de prueba desde la app de Instagram, y confirmar en `wrangler tail` que aparece `"Asesor humano le escribió a ... bot pausado"` y que el bot no contesta al siguiente mensaje de ese cliente durante `PAUSA_HORAS`.
